...
import type { DppAttachment } from '@/types/dpp';
...
const attachmentsList = ref<DppAttachment[]>([]);

const imageDialogVisible = ref<{
    visible: boolean;
    image_url: string;
}>({ visible: false, image_url: '' });

const showFileDialog = (attachment: DppAttachment) => {
...
<template>
    <div class="container">
        <DataTable :value="attachmentsList" tableStyle="min-width: 50rem">
            ...
            <Column header="File">
                <template #body="slotProps">
                    ...
                    <img :src="slotProps.data.image_thumbnail_url" :alt="slotProps.data.file_name"
                        class="w-6rem border-round" @click="showFileDialog(slotProps.data)" @error="imageLoadError" />
                </template>
            </Column>
        </DataTable>
        ...
        <!-- </Dialog> -->
        <Dialog v-model:visible="imageDialogVisible.visible" modal dismissableMask
            :style="{ width: 'auto', height: 'auto', maxWidth: '80vw', maxHeight: '80vh', overflow: 'hidden' }"
            :contentStyle="{ padding: '0' }" class="image-dialog">
            <img :src="imageDialogVisible.image_url" alt="Full-size Image" class="dialog-image" />
        </Dialog>
        <Dialog header="PDF Viewer" v-model:visible="displayModal" modal dismissableMask
            :style="{ width: 'auto', height: 'auto', maxWidth: '80vw', maxHeight: '80vh', overflow: 'hidden' }">
