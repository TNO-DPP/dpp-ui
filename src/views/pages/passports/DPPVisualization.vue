...
const getUrlQueryParams = async () => {
    ...
    uuidValue.value = route.params['uuid'] as string; // We assume that it is never a string array
...
const recursivelyCreateTreeTableData = async (dppUUID: string, prefix: string) => {
    let output = [];
    ...
    if (node.subpassports && node.subpassports.length > 0) {
        customNode.children = await Promise.all(
            node.subpassports.map(async (childNodeUUID: string, index: number) => {
...
const onClickId = async (item: DppTreeNode) => {
        ...
        <template #content>
            ...
            <div id="id-panel" class="pb-3">
                <Panel>
                    ...
                    <small id="starting-text">
                        <p>Start typing to immediately get recommendations for DPP IDs known in the database. Use the
                            filters to get filtered suggestions!</p>
                    </small>
                        ...
                        <Column field="Type" header="Type" :expander="true">
                            <template #body="slotProps">
                                <span class="pointer" @click="onClickId(slotProps.node)">
                                    <template v-if="slotProps.node?.data?.Type">{{ slotProps.node.data.Type
                                        }}</template>
                                    <template v-else><span class="unknownNode"><em>unknown</em></span></template>
                                </span>
                            </template>
                        </Column>
                        <Column field="Manufacturer" header="Manufacturer">
                            <template #body="slotProps">
                                <template v-if="slotProps.node?.data?.Manufacturer">{{ slotProps.node.data.Manufacturer
                                    }}</template>
                                <template v-else><span class="unknownNode"><em>unknown</em></span></template>
                            </template>
                        </Column>
